...
function RecipeDetail() {
    ...
    useEffect(() => {
        ...
        const fetchIngredients = async (idRecipe) => {
            try {
                const response = await fetch(`http://localhost:5000/api/Ingredients/ByRecipe/${idRecipe}`, {
                    method: 'GET', headers: {
                        'Content-Type': 'application/json',
                    },
                });
        ...
        const fetchSteps = async (idRecipe) => {
            try {
                const response = await fetch(`http://localhost:5000/api/Steps/ByRecipe/${idRecipe}`, {
                    method: 'GET', headers: {
                        'Content-Type': 'application/json',
                    },
                });
